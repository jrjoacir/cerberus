...
class ContractControllerTest < ActionDispatch::IntegrationTest
  test 'should create contract' do
    assert_difference('Contract.count', 1) do
      post '/contracts', params: {client_id: clients(:three).id, product_id: products(:three).id, enabled: true}.to_json, headers: headers
  ...
  test 'should raise unprocessable entity error on create a duplicate contract' do
    assert_no_difference('Contract.count') do
      post '/contracts', params: {client_id: clients(:one).id, product_id: products(:one).id, enabled: true}.to_json, headers: headers
