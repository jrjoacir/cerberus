...
class ClientsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should return clients filter by name' do
    get '/clients?name=Client-1'
    assert_response :success
  ...
  test 'should create a client' do
    assert_difference('Client.count', 1) do
      post '/clients', params: { name: 'Client-test' }.to_json, headers: headers
      assert_response :success
  ...
  test 'should raise unprocessable entity error on create a client' do
    assert_no_difference('Client.count') do
      post '/clients', params: { name: clients(:one).name }.to_json, headers: headers
  ...
  test 'should update a client' do
    assert_no_difference('Client.count') do
      ...
      put "/clients/#{clients(:one).id}", params: request_body.to_json, headers: headers
  ...
  test 'should raise unprocessable entity error on update a client with a duplicate name' do
    assert_no_difference('Client.count') do
      put "/clients/#{clients(:one).id}", params: { name: clients(:two).name }.to_json, headers: headers
      assert_equal @response.body, { message: 'Unprocessable Entity' }.to_json
      assert_equal @response.code, '422'
    end
  end
